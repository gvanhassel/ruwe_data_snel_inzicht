...
from typing import Callable, Dict, Iterator, List, Optional, Tuple, Protocol
...
def trainloop(
    ...
    if not tunewriter:
        log_dir = dir_add_timestamp(log_dir, model.name)
...
def save_config_to_file(config_dict, log_dir, file_name):
    def conver_value(i):
        try:
            return int(i)
    ...
    config_dict = {key: conver_value(value) for key, value in config_dict.items()}
    file_path = log_dir / f"{file_name}.txt"
